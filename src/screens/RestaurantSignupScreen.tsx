...
const RestaurantSignupScreen = ({ navigation}) => {
  ...
  const handleGetLocation = async () => {
    try {
      ...
      const response = await fetch(
        `https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=3dd8b1f1a60a4205a64d872fc068fe33`
      );
  ...
  const handleCreateAccount = async () => {
    if (!name || !contact || !street || !city || !state || !postalCode) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert('Success', 'Restaurant account created successfully!', [
          { text: 'OK', onPress: () => navigation.navigate('RestaurantDash') }
        ]);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.label}>Name of the Restaurant:</Text>
      <TextInput
        style={styles.input}
